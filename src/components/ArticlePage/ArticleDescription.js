  ...
  return !article ? null : (
    <>
      ...
      <Row className="mt-5">
        <Col xs={1} />
        <Col sm>
          <small className="ml-3" style={{fontSize:"20px"}}>{article.reporter.name}</small>
          <small className="ml-3" style={{fontSize:"20px"}}>{`${monthNames[date.getMonth()]
            } ${date.getDay()},${date.getFullYear()}`}</small>
          <small className="ml-3" style={{fontSize:"20px"}}>{article.location.city}</small>
